...
if (!empty($searchQuery)) {
    $products = array_filter($products, function ($product) use ($searchQuery) {
        return strpos(strtolower($product['dish_name']), $searchQuery) !== false;
    });
}
?>
...
<head>
    ...
    <title>Home - Catering Vietnam</title>
...
<body>
    ...
    <div class="container" id="product-container">
        <?php foreach ($products as $product): ?>
            <div class="box">
                <img src="../assets/uploads/menu_images/<?php echo htmlspecialchars($product['image']); ?>" alt="<?php echo htmlspecialchars($product['dish_name']); ?>">
                <div class="details">
                    <h2><?php echo htmlspecialchars($product['dish_name']); ?></h2>
                    <p class="price"><?php echo number_format($product['price'], 0, ',', '.'); ?>đ</p>
                    <form method="POST" action="" style="display: inline;">
                        <input type="hidden" name="product_name" value="<?php echo htmlspecialchars($product['dish_name']); ?>">
